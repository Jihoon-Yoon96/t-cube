...
현재 `layouts/builder.vue`는 사이드바, 모바일 헤더, 상단 툴바, 본문 영역을 구성하고, `useBuilderView()`의 currentView 상태에 따라 화면 구성을 제어한다.
...
컴포넌트는 화면 표현과 사용자 이벤트 연결을 중심으로 작성하고, 상태 조합, 파싱, 변환, 외부 API 처리 같은 로직은 composable, store, service, server 계층으로 분리한다.
...
`composables`는 Vue/Nuxt에서 상태나 로직을 재사용 가능한 함수로 묶는 계층이다. 일반적으로 `useSomething()` 형태를 사용하며, `ref`, `computed`, `watch`, 브라우저 API, 비동기 요청 상태, 사용자 인터랙션 흐름처럼 Vue 반응형 시스템과 가까운 로직을 다룬다.

이 프로젝트에서 composable은 관심사별로 나뉘어 컴포넌트가 필요한 기능만 직접 호출할 수 있도록 구성한다. `stores`가 공유 상태의 원천이라면 `composables`는 그 상태를 사용해 업로드 분석, HTML 생성, 화면 이동 guard, 레이아웃 캔버스 조작 같은 작업을 실행한다.
...
- `composables/layout/useBuilderLayoutDesignToHtml.ts`: 레이아웃 블록 기반 HTML 생성
- `composables/editor/useBuilderEditor.ts`: HTML 편집 문서, 선택 요소, dirty 상태와 문서 수정 API
- `composables/navigation/useBuilderNavigationGuard.ts`: 화면 이동 guard와 생성 중 요청 이탈 확인

composable에 두기 좋은 로직:

- 여러 store 상태를 함께 읽거나 변경하는 기능 흐름
- API 호출, 취소, loading/error 상태 전환을 포함하는 사용자 작업
- 화면 이동 전 확인, 업로드 분석, 편집기 반영처럼 컴포넌트보다 큰 단위의 interaction
- Vue 반응형 상태와 가까운 로직이지만 순수 service로 분리하기에는 앱 흐름에 의존하는 코드

composable에 두지 않는 것이 좋은 로직:

- 입력값만 받아 결과를 반환하는 순수 변환 로직
- 서버에서만 실행해야 하는 외부 API key 기반 처리
- 특정 컴포넌트 안에서만 끝나는 단순 표시 상태
...
현재 빌더는 store의 currentView 상태를 기준으로 화면을 전환한다. 컴포넌트에서는 직접 store setter를 호출하기보다 `useBuilderView().moveToView()`처럼 guard가 적용된 이동 API를 사용한다. 여기서 view는 Nuxt page route가 아니라 `BuilderApp` 안에서 렌더링할 화면 단위를 의미한다.
...
→ design-method / layout-design / ai-prompt-design
→ preview
```
...
```txt
FileDropzone
→ useBuilderUpload / builder store
→ useBuilderFileAnalysis
...
```txt
LayoutBuilder
→ useBuilderLayoutCanvas
→ useBuilderLayoutDesignToHtml
→ HTML 생성
→ editor state
→ HTML editor / preview
```
