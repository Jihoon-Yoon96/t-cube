...
현재 `layouts/builder.vue`는 사이드바, 모바일 헤더, 상단 툴바, 본문 영역을 구성하고, Pinia store의 step 상태에 따라 화면 구성을 제어한다.
...
- `components/builder/steps/create-template`: 새 템플릿 생성 플로우
- `components/builder/steps/edit-template`: 기존 템플릿 수정 플로우
...
- `stores/builder/step.ts`: 현재 작업 단계
...
현재 빌더는 store의 step 상태를 기준으로 화면을 전환한다.
