...
이 프로젝트에서 composable은 store 상태를 직접 소유하기보다 여러 상태 모듈을 조합해 하나의 기능 흐름을 수행한다. 즉, `stores`가 상태의 원천이라면 `composables`는 그 상태를 사용해 업로드 분석, HTML 생성, 화면 이동 guard 같은 workflow를 실행한다.

현재 빌더 관련 composable:

- `composables/navigation/useBuilderNavigationGuard.ts`: 화면 이동 guard와 생성 중 요청 이탈 확인
- `composables/file/useBuilderFileAnalysis.ts`: 업로드 파일 유형별 분석 시작과 화면 전환
- `composables/html/useBuilderDesignToHtml.ts`: 이미지/PDF 기반 HTML 생성 API 호출과 취소
- `composables/layout/useBuilderLayoutCanvas.ts`: 레이아웃 작성 화면의 블록 데이터와 선택 상태 관리
- `composables/layout/useBuilderLayoutDesign.ts`: 레이아웃 블록 기반 HTML 생성
